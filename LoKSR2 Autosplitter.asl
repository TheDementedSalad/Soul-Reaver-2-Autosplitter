...
split
{
	if(settings["Gless"]){
		...
		if(current.Cutscene == 4 && current.Reavers == 28 && current.X >= -480f && current.X <= -460f && current.Y >= -1760f && current.Y <= -1750f && current.Z >= -2075f && current.Z <= -2060f && vars.split == 0){
			if(settings["22"]){
				vars.split2++;
				return true;
			}
